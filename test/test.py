# 標準函式庫
import os #用來與作業系統互動
import sys #與Python解譯器互動
import json #JSON處理模組，用來儲存/讀取設定檔
from datetime import date, datetime #date：處理年月日；datetime：可以包含年月日時分秒，用來產生「現在時間」或格式化

# 第三方套件
from fpdf import FPDF, XPos, YPos #FPDF：主要的PDF類別；XPos、YPos：用來控制位置對齊用的值（須為fpdf2）
import win32print #Windows系統專屬模組，通常用於列印功能

# Tkinter GUI
import tkinter as tk # 匯入tkinter命名為tk，這是Python內建的GUI套件，用來建立圖形介面視窗
from tkinter import ttk, filedialog, messagebox #ttk：較美觀的UI元件；filedialog：開啟/儲存檔案；messagebox：彈出提示訊息或警告視窗


def resource_path(relative_path): # 定義一個函式 resource_path()，輸入是相對路徑 relative_path(不管你是在原始碼執行還是打包成 .exe 執行，都能正確載入外部資源)
    """取得打包後與開發時都適用的資源路徑"""
    if hasattr(sys, '_MEIPASS'): #PyInstaller在打包.exe時，會把資源解壓到一個臨時資料夾，這個資料夾的路徑會儲存在sys._MEIPASS
        return os.path.join(sys._MEIPASS, relative_path) # 如果偵測到sys有這個屬性，就代表現在是在「打包後執行」的環境，應該從 _MEIPASS 開始找檔案。
    return os.path.join(os.path.abspath("."), relative_path) #否則表示你在開發階段，就從目前資料夾（"."）去找資源

FONT_PATH = resource_path("msjh.ttc") #有一字型檔 msjh.ttc（微軟正黑體）在你的專案資料夾中，就可以這樣取得它的路徑(不用管是在打包前還是打包後)

class ReceiptApp: # 定義一個GUI類別(ReceiptApp)，用來管理整個應用程式的視窗與功能。
    def __init__(self, root):   
        self.root = root # root：外部傳進來的tk.Tk() 物件(主視窗)。 
        self.root.title("收據/發票輸入介面 - (新版)") # self.root.title()： 設定視窗標題為「收據/發票輸入介面 - (新版)」
        self.logo_path = resource_path("logo.png") # 儲存logo圖片的絕對路徑（無論是開發中或打包後）。

        self.printers = [printer[2] for printer in win32print.EnumPrinters(2)] # win32print.EnumPrinters(2)：列出所有可用的印表機；printer[2]：是每台印表機的名稱
        self.selected_printer = tk.StringVar(value=win32print.GetDefaultPrinter()) # 建立一個StringVar變數來儲存選取中的印表機，並預設為目前系統的預設印表機

        frame_printer = tk.LabelFrame(root, text="選擇印表機") # 建立一個區塊（LabelFrame）來放選擇印表機相關元件
        frame_printer.grid(row=7, column=0, columnspan=2, padx=10, pady=5, sticky="ew") # grid：定位在第 7 行，跨 2 欄；sticky="ew"：元件會沿著左右延展
        tk.Label(frame_printer, text="印表機：").pack(side="left") # 這個區塊左邊放一個標籤顯示「印表機：」
        self.printer_menu = ttk.Combobox(frame_printer, textvariable=self.selected_printer, values=self.printers, width=50) # ttk.Combobox：建立一個下拉式選單讓使用者選擇印表機；
